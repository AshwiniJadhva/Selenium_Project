...
public class DropdownPractice {
	...
	public static void main(String[] args) throws InterruptedException {
		 ...
		 * selectDD.deselectAll(); for(WebElement weListItem : listAllOptions) {
		 * 
		 * if(Integer.valueOf(weListItem.getText()) % 2 == 0) {
		 * selectDD.selectByVisibleText(weListItem.getText());
		 * 
		 * } System.out.println(weListItem.getText()
		 * +" Selected Status :"+weListItem.isSelected()); }
		 */

		/*
		 * System.out.
		 * println("SEVENTH PS - Get all selected unselected and unselecetd Selected******"
		 * ); List<WebElement> allSelectedList = selectDD.getAllSelectedOptions();
		 * listAllOptions.removeAll(allSelectedList); selectDD.deselectAll();
		 * for(WebElement we : listAllOptions) {
		 * selectDD.selectByVisibleText(we.getText()); //System.out.println(we.getText()
		 * +" Selected Status :"+we.isSelected()); } List<WebElement> finalList =
		 * selectDD.getOptions(); for(WebElement fWE : finalList) {
		 ...
		 * System.out.println(finalList.get(finalList.size()-2).getText()+
		 * " Selected status : " +finalList.get(finalList.size()-2).isSelected());
