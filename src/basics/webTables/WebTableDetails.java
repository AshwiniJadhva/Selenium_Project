...
public class WebTableDetails {
	...
	void printFNameOfEmployee(String empCity) {
		...
		List<WebElement>pageNumbers = driver.findElements(By.xpath("//button[@class='dt-paging-button disabled previous']//following::button[@class='dt-paging-button']"));
		
		for(WebElement e : pageNumbers) {
			System.out.println(e.getText());
		}
		
		int totalepageCount = pageNumbers.size();
		for (int index = 1; index < totalepageCount; index++) {
		...
		/*
		 * Object[] namesArr = nameList.toArray(); for (int innerIndex = 0; innerIndex <
		 * namesArr.length; innerIndex++) {
		...
		//webTableDetails.printAgeOfEmployee("Caesar Vance");
		  ...
		  //webTableDetails.printFNameOfEmployee("London");
			 ...
			 // webTableDetails.getEmpNameWorkingAs("Software Engineer");
			  ...
			  //webTableDetails.getEmpAgeOfMinSalary(); webTableDetails.getRowCount(1);
